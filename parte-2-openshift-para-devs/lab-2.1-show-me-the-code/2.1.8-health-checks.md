...
```bash
oc get dc -n <nome-do-projet-no-openshift>
```
...
oc set probe dc/workshop-ocp \
 --readiness \
 --get-url=http://:8080/readiness.php -n <nome-do-projeto-no-openshift>
```
...
oc set probe dc/workshop-ocp \
 ...
 --liveness \
 --get-url=http://:8080/liveness.php -n <nome-do-projeto-no-openshift>
```
...
> É importante que você tenha somente uma instância da aplicação rodando para quando clicar dentro do círculo ele vá para a página certa. Para garantir isso, execute o seguinte comando: `oc scale dc/workshop-ocp --replicas=1`
